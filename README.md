...
| `exact_connect` | `false` | with `relax_uc: true`, keep the village grid-connection binaries **exact** while the unit-commitment binaries relax. Plain `relax_uc` also relaxes `vVIL_CONNECT`, so a fractional connection pays a fraction of the wire cost for full trade benefit and `Connected` becomes a rounding artifact. The exact-wire / relaxed-UC combination is the tractable form of the coordination question (~780 binaries instead of ~122k on the Timor market case). No-op when `relax_uc` is false. |
| `connect_pattern` | `""` | fix-and-verify: path to a CSV with `ID` and `Connected` columns — a landed run's `site_connection_results.csv` works verbatim — whose 0/1 pattern is **fixed** onto `vVIL_CONNECT` before the solve. Use it to price a connection plan found on a reduced-time-resolution dataset (`tools/make_reduced_weeks.py`) at full resolution: with `relax_uc: true` the remaining problem is an LP, and one solve gives the exact full-resolution cost of that concrete plan with no aggregation caveat. Errors on a missing file, a row-count mismatch, or a non-0/1 value; requires a `Grid` + `VillageBuild` scenario. Empty = off. |
| `start_pattern` | `""` | seeded warm start: same CSV format as `connect_pattern`, but the 0/1 values are **MIP start values** on `vVIL_CONNECT`, not fixes — the search may move off them. Seed a pattern search from a known-good plan (a fix-and-verify winner) so the reported incumbent can never be worse than that plan; the default start is all-islanded. Ignored when `connect_pattern` is set. Errors on a missing file, a row-count mismatch, or a non-0/1 value. Empty = off. |
