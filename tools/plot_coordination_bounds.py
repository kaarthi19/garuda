...
TIMOR = dict(off=69.134350, on_lb=68.967491)                    # final
MKT = dict(off_ub=96.886109, off_lb=96.812863, on_lb=59.274058)  # preliminary
...
def main(argv=None):
    ...
    m_lo = max(0.0, MKT["off_lb"] - inc) if inc is not None else 0.0
    on_lb = bound if bound is not None else MKT["on_lb"]
    m_hi = MKT["off_ub"] - on_lb
    status = "landed" if inc is not None else "runs in flight"
    ...
    ax.annotate(f"≤ ${m_hi:.1f} M/yr — preliminary upper bound\n"
                f"(islanded cost − coordinated root LP; {status})",
                xy=(m_hi, 1 + bar_h / 2), xytext=(m_hi - 1.2, 1 + 0.40),
    ...
    fig.text(0.20, 0.115,
             f"as of {stamp} · timor__marketfix is a sensitivity dataset (derived grid demand; "
             "corrected RE costs; DMO coal)\n"
             "upper bound uses the LP relaxation — the integrality gap on this formulation has "
             "measured ~55%, so the true value\nlikely sits well below the light bar's end · "
             "per-village trade allocation is degenerate at 0/0 prices (totals unaffected)",
