...
# UI strings for the rendered report (--lang). Column names in the tables stay
# canonical (they are the documented output schema — see docs/outputs_guide.md);
# titles, labels, captions and the footer are translated. Terminology follows
# common ESDM / PLN usage (bauran, kapasitas terpasang, energi terbarukan,
# energi tak terlayani); the Indonesian strings are a careful draft — partner
# review welcome.
STRINGS = {
    "en": {
        "html_title": "Garuda report",
        "cost": "Total system cost", "co2": "CO₂ emissions", "re": "Grid RE share",
        "demand": "Annual demand", "served": "Energy served",
        "unserved": "Unserved energy", "unserved_pct": "Unserved share",
        "unserved_annual": "Unserved energy (annual)",
        "gen_ylabel": "Generation (GWh, representative periods)",
        "gen_title": "Generation mix by technology",
        "gen_cap": ("Generation by technology over the representative periods "
                    "(not annualised — Sub_Weights vary by period)."),
        "cap_ylabel": "Capacity (MW)",
        "cap_title": "Installed capacity by technology",
        "cap_cap": "Installed capacity by technology (post-solve).",
        "cost_xlabel": "Cost (M$)",
        "cost_title": "Cost breakdown",
        "cost_cap": "System cost decomposition (M$/yr).",
        "rel_ylabel": "Unserved (% of zone demand)",
        "rel_title": "Per-zone reliability shortfall",
        "rel_cap": "Unserved energy share by zone (dispatch / reliability run).",
        "tbl_rel": "Per-zone reliability",
        "tbl_captech": "Capacity & generation by technology",
        "eng_dispatch": "dispatch / reliability", "eng_expansion": "capacity expansion",
        "subtitle": "{clean} case · {engine} run · folder {name}",
        "footer": ("Generated by Garuda <code>tools/report.py</code> from {name}. "
                   "Cost (M$/yr), CO₂, RE share, demand and unserved energy are "
                   "annualised over the representative periods; the generation-mix "
                   "chart shows the representative-period sum (Sub_Weights vary, so "
                   "it is not annualised)."),
    },
    "id": {
        "html_title": "Laporan Garuda",
        "cost": "Total biaya sistem", "co2": "Emisi CO₂",
        "re": "Pangsa energi terbarukan (jaringan)",
        "demand": "Permintaan tahunan", "served": "Energi terlayani",
        "unserved": "Energi tak terlayani", "unserved_pct": "Porsi tak terlayani",
        "unserved_annual": "Energi tak terlayani (tahunan)",
        "gen_ylabel": "Pembangkitan (GWh, periode representatif)",
        "gen_title": "Bauran pembangkitan per teknologi",
        "gen_cap": ("Pembangkitan per teknologi selama periode representatif "
                    "(belum disetahunkan — Sub_Weights berbeda antar periode)."),
        "cap_ylabel": "Kapasitas (MW)",
        "cap_title": "Kapasitas terpasang per teknologi",
        "cap_cap": "Kapasitas terpasang per teknologi (hasil optimisasi).",
        "cost_xlabel": "Biaya (juta US$)",
        "cost_title": "Rincian biaya",
        "cost_cap": "Dekomposisi biaya sistem (juta US$/tahun).",
        "rel_ylabel": "Tak terlayani (% permintaan zona)",
        "rel_title": "Defisit keandalan per zona",
        "rel_cap": "Porsi energi tak terlayani per zona (run dispatch / keandalan).",
        "tbl_rel": "Keandalan per zona",
        "tbl_captech": "Kapasitas & pembangkitan per teknologi",
        "eng_dispatch": "dispatch / keandalan", "eng_expansion": "ekspansi kapasitas",
        "subtitle": "kasus {clean} · run {engine} · folder {name}",
        "footer": ("Dibuat oleh Garuda <code>tools/report.py</code> dari {name}. "
                   "Biaya (juta US$/tahun), CO₂, pangsa energi terbarukan, permintaan "
                   "dan energi tak terlayani disetahunkan atas periode representatif; "
                   "grafik bauran pembangkitan menunjukkan jumlah periode "
                   "representatif (Sub_Weights berbeda antar periode, sehingga belum "
                   "disetahunkan). Nama kolom tabel mengikuti skema keluaran kanonik "
...
def metrics(R, ad_gwh=None, lang="en"):
    ...
    if "cost_results" in R and "Total_Costs" in R["cost_results"]:
        m["cost"] = (float(R["cost_results"]["Total_Costs"].iloc[0]), "M$", L["cost"])
    if "clean_energy_results" in R:
        ce = R["clean_energy_results"]
        if "CO2_Emissions" in ce:
            m["co2"] = (float(ce["CO2_Emissions"].iloc[0]) / 1e3, "ktCO₂", L["co2"])
        if "Grid_REShare" in ce:
            m["re"] = (100 * float(ce["Grid_REShare"].iloc[0]), "%", L["re"])
    ...
    if ad_gwh is not None:
        m["demand"] = (ad_gwh, "GWh", L["demand"])
        if unserved_gwh is not None:
            served = max(ad_gwh - unserved_gwh, 0.0)
            m["served"] = (served, "GWh", L["served"])
            m["unserved"] = (unserved_gwh, "GWh", L["unserved"])
            m["unserved_pct"] = (100 * unserved_gwh / ad_gwh if ad_gwh else 0.0, "%", L["unserved_pct"])
    elif unserved_gwh is not None:
        m["unserved"] = (unserved_gwh, "GWh", L["unserved_annual"])
    return m
...
def figures(R, meta, lang="en"):
    ...
    if mix is not None:
        ...
        ax.set_ylabel(L["gen_ylabel"])
        ax.set_title(L["gen_title"])
        ax.tick_params(axis="x", rotation=35)
        for lbl in ax.get_xticklabels():
            lbl.set_ha("right")
        figs.append((L["gen_cap"], fig))

    cap = _by_tech(gen, "Total_MW")
    if cap is not None:
        fig, ax = plt.subplots(figsize=(7, 4))
        ax.bar(cap.index, cap.values, color=[_color(t) for t in cap.index])
        ax.set_ylabel(L["cap_ylabel"])
        ax.set_title(L["cap_title"])
        ax.tick_params(axis="x", rotation=35)
        for lbl in ax.get_xticklabels():
            lbl.set_ha("right")
        figs.append((L["cap_cap"], fig))
    ...
    if cost is not None and len(cost):
        ...
        if items:
            ...
            ax.set_title(L["cost_title"])
            figs.append((L["cost_cap"], fig))
    ...
    if rel is not None and len(rel) > 1 and "NSE_Percent_of_Demand" in rel:
        ...
        ax.set_ylabel(L["rel_ylabel"])
        ax.set_title(L["rel_title"])
        ax.tick_params(axis="x", rotation=35)
        for lbl in ax.get_xticklabels():
            lbl.set_ha("right")
        figs.append((L["rel_cap"], fig))
    return figs


def _tables(R, lang="en"):
    """Key tables as (title, DataFrame). Column names stay canonical (schema)."""
    L = STRINGS[lang]
    out = []
    rel = R.get("reliability_results")
    if rel is not None and len(rel):
        ...
        out.append((L["tbl_rel"], rel[cols].round(2)))
    gen = R.get("generator_results")
    if gen is not None and "GWh" in gen.columns and "technology" in gen.columns:
        ...
        out.append((L["tbl_captech"], t.round(1).reset_index()))
    return out


HTML_TMPL = """<!doctype html><html lang="{{ lang }}"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{{ html_title }} — {{ name }}</title>
<style>
...
def render_html(R, meta, out_path, ad_gwh=None, lang="en"):
    from jinja2 import Template
    L = STRINGS[lang]
    m = metrics(R, ad_gwh, lang)
    figs = [(cap, _fig_to_b64(fig)) for cap, fig in figures(R, meta, lang)]
    cards = []
    for k in CARD_ORDER:
        if k in m:
            v, u, lbl = m[k]
            cards.append(dict(value=f"{v:,.1f}", unit=u, label=lbl))
    tables = [(t, df.to_html(index=False, border=0)) for t, df in _tables(R, lang)]
    eng = L["eng_dispatch"] if "reliability_results" in R else L["eng_expansion"]
    title = f"{meta['scenario'] or 'scenario'} · {meta['island'] or meta['name']} · {meta['year']}".strip(" ·")
    subtitle = L["subtitle"].format(clean=meta["clean"], engine=eng, name=meta["name"])
    html = Template(HTML_TMPL).render(title=title, subtitle=subtitle, name=meta["name"],
                                      cards=cards, figures=figs, tables=tables,
                                      lang=lang, html_title=L["html_title"],
                                      footer=L["footer"].format(name=meta["name"]))
    with open(out_path, "w") as fh:
        fh.write(html)
    return out_path


def render_pdf(R, meta, out_path, html_path=None, ad_gwh=None, lang="en"):
    ...
    with PdfPages(out_path) as pdf:
        ...
        eng = L["eng_dispatch"] if "reliability_results" in R else L["eng_expansion"]
        fig.text(0.07, 0.905, L["subtitle"].format(clean=meta["clean"], engine=eng,
                                                   name=meta["name"]),
        ...
        for cap, f in figs:
            ...
            plt.close(f)
        # key tables
        for title, df in _tables(R, lang):
...
def main(argv):
    ...
    ap.add_argument("--lang", default="en", choices=sorted(STRINGS),
                    help="report language: en (English) or id (Bahasa Indonesia)")
    ...
    html_path = args.html or os.path.join(args.results_dir, f"report{suffix}.html")
    render_html(R, meta, html_path, ad_gwh=ad, lang=args.lang)
    print(f"wrote {html_path}")

    if not args.no_pdf:
        pdf_path = args.pdf or os.path.join(args.results_dir, f"report{suffix}.pdf")
